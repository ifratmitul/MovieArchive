...
describe('MovieService', () => {
  ...
  it('should return top 5 movie Observable<MoviesDetails[]>', () => {
    ...
    const req = httpMock2.expectOne(`${movieConfig.latestMovieEndPoint}?language=${movieConfig.defaultLanguage}&api_key=${baseConfig.api_key}`);
    expect(req.request.method).toBe('GET');
    req.flush(mockApiResponse);
  });
  ...
  it('should return movie details Observable<MoviesDetails[]>', () => {
    ...
    const req = httpMock3.expectOne(`${movieConfig.movieDetailsEndPoint}/${mockApiResponse.id}?api_key=${baseConfig.api_key}`);
    expect(req.request.method).toBe('GET');
    req.flush(mockApiResponse);
  });
});
